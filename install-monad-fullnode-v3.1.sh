...
# v3.1 (FIX 12): voting check is meaningful only for actual validators.
# A fullnode logs "vote successful" as part of local consensus state machine,
# but its votes are not counted (self_stake_bps = 0, not in validators.toml).
# We probe self_stake_bps via control panel — if 0, this node is not a
# validator and the voting metric is skipped entirely (no false NOT_VOTING).
SELF_STAKE=$(monad-debug-node --control-panel-ipc-path /home/monad/monad-bft/controlpanel.sock metrics 2>/dev/null \
  | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('Read',{}).get('GetMetrics',{}).get('Response',{}).get('node_state',{}).get('self_stake_bps',0))" 2>/dev/null \
  || echo 0)
if [ "${SELF_STAKE:-0}" -gt 0 ] 2>/dev/null; then
...
# v3.1 (FIX 12): voting is meaningful only for actual validators.
# Probe self_stake_bps via control panel; if 0, this is a fullnode and the
# "vote successful" log lines are local consensus state machine events that
# don't represent real network voting (false positive).
SELF_STAKE=$(monad-debug-node --control-panel-ipc-path /home/monad/monad-bft/controlpanel.sock metrics 2>/dev/null \
  | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('Read',{}).get('GetMetrics',{}).get('Response',{}).get('node_state',{}).get('self_stake_bps',0))" 2>/dev/null \
  || echo 0)
if [ "${SELF_STAKE:-0}" -gt 0 ] 2>/dev/null; then
...
# v3.1 (FIX 12): show ℹ️ for fullnode (n/a), ⚠️ only when validator isn't voting
if [ "${SELF_STAKE:-0}" -le 0 ] 2>/dev/null; then I_VOTE="ℹ️"
elif [ "$VOTING" -gt 0 ]; then I_VOTE="✅"
else I_VOTE="⚠️"
fi
...
" | while IFS='|' read -r U_ID RCHAT TEXT; do
    # v3.1 (FIX 13): variable name was 'UID' which is bash readonly →
    # `read` failed silently, bot couldn't process /status. Renamed to U_ID.
    echo $((U_ID + 1)) > "$OFFSET_FILE"
