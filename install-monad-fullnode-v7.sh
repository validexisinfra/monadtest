...
#
# Changelog v6 → v7:
#   - FIX 13 (CRITICAL): Phase 4 keystore failure on fresh install.
#     Apt-pkg creates /home/monad/monad-bft/config as root:root 0755;
#     `sudo -u monad monad-keystore create --keystore-path .../id-secp`
#     hit Permission denied. Now chown -R monad:monad ${CONFIG_DIR} at
#     the start of phase4_keystore (after Phase 2 apt may rewrite owner).
#   - FIX 15: ENABLE_ON_BOOT default changed false → true. A production
#     fullnode should restart with the host. Opt-out via --no-enable-on-boot
#     for rare cases (validator promotion windows, planned drains).
#   - FIX 16: /etc/monad ownership root:monad 0775 in Phase 1 so future
#     companion scripts (monad-update.sh etc.) can write there without
#     full root.
#   - FIX 17: Telegram bootstrap delivery is now verified in Phase 9.
#     If getMe returns non-ok within 10s after tg_notify, log a warning
#     so the operator knows alerts may be misconfigured.
#   - FIX 18: phase7_monitoring moved AFTER phase8_start. Previously cron
#     monad-monitor.sh fired during Phase 7.5 snapshot download and
#     spammed false "service down" alerts. Order is now:
#       6_systemd → 7_5_snapshot → 8_start → 7_monitoring → 9_verify
#     Monitoring activates only after the node is up AND before the
#     bootstrap-complete TG notification — its "Monitoring: ACTIVE"
#     claim is accurate at send time.
#
# Changelog v5 → v6:
#   - FIX bug in monad-bot-handler.sh: `read -r UID` failed because UID
#     is a bash readonly built-in (current user's id) — bot never
#     processed any /status commands. Renamed loop var to UPD_ID.
#   - phase7: auto-activate monitoring out-of-the-box. Cron files written
#     directly (no .disabled suffix), monad-bot.service enabled+started.
#     Removes the "manual activation" footnote — install is now fully
#     hands-off including monitoring.
#
# Changelog v4 → v5:
#   - monad-monitor.sh: add BFT watchdog. Detects monad-bft dead-state
#     (block-stuck >=9min OR RPC-down >=9min while BFT systemctl-active)
#     and auto-runs `systemctl restart monad-bft`. Guards: 5min warmup,
#     30min cooldown between auto-restarts. Sends TG alert on fire.
#     Background: observed 2026-05-05 on Astrosynx — bft was systemd-active
#     but UDP transmission stopped, peer-discovery sat idle with empty
#     chosen_peers; manual restart cleared it within 60s.
#
# Changelog v3 → v4:
#   - Add phase2_5_otel: install otelcol-contrib (one of 6 required
#     services per official Monad docs); fixes 4317 ConnectionRefused
#     log spam and exposes Prometheus metrics on :8889.
#   - phase1: add `ufw allow 8000/udp` (raptorcast) — was missing.
#
# Usage:
...
#   --skip-snapshot       Skip Phase 7.5 snapshot restore (sync from genesis)
#   --enable-on-boot      Enable systemd auto-start on boot (DEFAULT in v7)
#   --no-enable-on-boot   Opt-out of auto-start on boot (v7+)
...
OTELCOL_SHA256="7d4356adf690b1a08f5d536331dc932ee28d31fd80801b6e960cd6b5618f86db"  # otelcol-contrib_0.139.0_linux_amd64.tar.gz (verified from official checksums.txt)
...
SKIP_SNAPSHOT=false
ENABLE_ON_BOOT=true   # v7 (FIX 15): default true — production fullnode should restart with the host. Opt-out via --no-enable-on-boot.
ASSUME_YES=false
TRIEDB_DEVICE=""
NODE_NAME=""
SELF_IP=""
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --no-enable-on-boot) ENABLE_ON_BOOT=false; shift ;;
...
phase1_system() {
  ...
  # NOTE: aria2 added in v3 — required by Monad Foundation snapshot script
  # (restore-from-snapshot.sh uses aria2c for multi-connection downloads).
  apt-get install -y --no-install-recommends \
    curl nvme-cli jq ufw fail2ban aria2 \
  ...
  # v7 (FIX 16): /etc/monad readable+writable by monad group, so future
  # companion scripts (monad-update.sh, key-rotate, etc.) can write here
  # without escalating to full root. keystore.env inside is still 0600 root:root.
  chown root:monad /etc/monad
  chmod 0775 /etc/monad
  ...
  if command -v ufw &>/dev/null; then
    ...
    ufw allow 8000/udp comment "Monad raptorcast"
    ufw allow 8001/udp comment "Monad Auth"
    ufw --force enable
    log_ok "UFW configured (SSH:${SSH_PORT}, P2P:8000/tcp+udp, Auth:8001/udp)"
  fi
...
# ═══════════════════════════════════════════════════════════════════════
# PHASE 2.5: OpenTelemetry Collector  (NEW in v4)
# ═══════════════════════════════════════════════════════════════════════
# Per official Monad docs (docs.monad.xyz/node-ops/full-node-installation),
# `otelcol` is one of the six required services. Without it, monad-bft and
# monad-rpc spam "127.0.0.1:4317 ConnectionRefused" every ~500ms because
# they try to ship metrics via OTLP gRPC. Installing otelcol-contrib pins
# the receiver and exposes Prometheus metrics on :8889.
#
# We install the official otelcol-contrib release binary (verified by
# SHA256 from the upstream checksums.txt). The Monad .deb already ships
# /opt/monad/scripts/otel-config.yaml — we point the unit at that file.
# ═══════════════════════════════════════════════════════════════════════
phase2_5_otel() {
  log "═══ PHASE 2.5: OpenTelemetry Collector ═══"

  local OTELCOL_TGZ="otelcol-contrib_${OTELCOL_VERSION}_linux_amd64.tar.gz"
  local OTELCOL_URL="https://github.com/open-telemetry/opentelemetry-collector-releases/releases/download/v${OTELCOL_VERSION}/${OTELCOL_TGZ}"
  local OTELCOL_BIN="/usr/local/bin/otelcol-contrib"
  local OTELCOL_CFG="/opt/monad/scripts/otel-config.yaml"

  # Idempotency: if correct version already installed and unit running, skip
  if [[ -x "$OTELCOL_BIN" ]]; then
    local installed_ver
    installed_ver=$("$OTELCOL_BIN" --version 2>/dev/null | awk '{print $NF}' | tr -d 'v' || echo "")
    if [[ "$installed_ver" == "$OTELCOL_VERSION" ]]; then
      log_ok "otelcol-contrib v${OTELCOL_VERSION} already installed"
    else
      log_warn "otelcol-contrib v${installed_ver:-unknown} found; replacing with v${OTELCOL_VERSION}"
      install_otelcol_binary "$OTELCOL_TGZ" "$OTELCOL_URL" "$OTELCOL_BIN"
    fi
  else
    install_otelcol_binary "$OTELCOL_TGZ" "$OTELCOL_URL" "$OTELCOL_BIN"
  fi

  # The Monad .deb provides the config at /opt/monad/scripts/otel-config.yaml.
  # If missing (defensive), fail loudly — we don't ship our own to avoid drift.
  [[ -f "$OTELCOL_CFG" ]] || fail "otel config not found at $OTELCOL_CFG (Monad .deb should provide it)" "Phase 2.5"

  # Dedicated unprivileged user for otelcol (least privilege; cannot read keystores)
  if ! id otelcol &>/dev/null; then
    useradd --system --no-create-home --shell /usr/sbin/nologin otelcol
    log_ok "Created system user otelcol"
  fi

  # systemd unit
  cat > /etc/systemd/system/otelcol.service << OTELUNIT
[Unit]
Description=OpenTelemetry Collector (Monad metrics)
...
User=otelcol
Group=otelcol
ExecStart=${OTELCOL_BIN} --config=${OTELCOL_CFG}
Restart=on-failure
RestartSec=10
LimitNOFILE=65536
...
OTELUNIT
  ...
  systemctl enable otelcol.service >> "$LOGFILE" 2>&1
  systemctl restart otelcol.service

  sleep 3
  if ! systemctl is-active --quiet otelcol; then
    log_err "otelcol failed to start — last 20 log lines:"
    journalctl -u otelcol -n 20 --no-pager 2>&1 | tee -a "$LOGFILE"
    fail "otelcol service inactive after start" "Phase 2.5"
  fi

  log_ok "otelcol-contrib v${OTELCOL_VERSION} active (Prometheus metrics on :8889)"
}

# Helper: download, verify SHA256, extract otelcol binary
# Note: explicit cleanup (no `trap ... RETURN`) — RETURN traps persist
# across subsequent function returns by default in bash and would refire
# with the local `tmpdir` out of scope, triggering `set -u` error.
install_otelcol_binary() {
  local tgz="$1" url="$2" target="$3"
  local tmpdir
  tmpdir=$(mktemp -d)

  log "Downloading ${tgz}..."
  if ! curl -fsSL --max-time 120 -o "${tmpdir}/${tgz}" "$url"; then
    rm -rf "$tmpdir"
    fail "Failed to download otelcol from ${url}" "Phase 2.5"
  fi

  log "Verifying SHA256..."
  local actual_sha
  actual_sha=$(sha256sum "${tmpdir}/${tgz}" | awk '{print $1}')
  if [[ "$actual_sha" != "$OTELCOL_SHA256" ]]; then
    rm -rf "$tmpdir"
    fail "SHA256 mismatch for ${tgz}: expected ${OTELCOL_SHA256}, got ${actual_sha}" "Phase 2.5"
  fi
  log_ok "SHA256 verified: ${actual_sha}"

  if ! tar -xzf "${tmpdir}/${tgz}" -C "$tmpdir" otelcol-contrib; then
    rm -rf "$tmpdir"
    fail "Failed to extract otelcol-contrib from ${tgz}" "Phase 2.5"
  fi
  install -m 0755 -o root -g root "${tmpdir}/otelcol-contrib" "$target"
  rm -rf "$tmpdir"
  log_ok "otelcol-contrib installed at ${target}"
}
...
phase4_keystore() {
  ...
  # v7 (FIX 13): apt postinst creates ${CONFIG_DIR} as root:root 0755 in
  # Phase 2, which makes `sudo -u monad monad-keystore create
  # --keystore-path ${CONFIG_DIR}/id-secp` fail with PermissionDenied.
  # Take ownership before any monad-user write. Idempotent on re-runs.
...
phase7_monitoring() {
  ...
  # ─── monad-monitor.sh (v5: with BFT watchdog) ───
...
RPC_DOWN_COUNT_FILE="/tmp/monad-rpc-down-count"
WATCHDOG_LAST_RESTART_FILE="/tmp/monad-watchdog-last-restart"
touch "$ALERT_FILE"

# ─── Watchdog config (v5) ───
WATCHDOG_COOLDOWN_SEC=1800        # 30 min between auto-restarts (avoid loops)
WATCHDOG_MIN_UPTIME_SEC=300       # require 5 min BFT uptime before any restart
WATCHDOG_STUCK_THRESHOLD=3        # cycles (3 min each) ⇒ ~9 min block-stuck
WATCHDOG_RPC_DOWN_THRESHOLD=3     # cycles ⇒ ~9 min RPC-down
...
# ─── RPC + block-stuck tracking (separate counters for watchdog) ───
...
if [ -z "$RPC_CHECK" ]; then
  ...
  RPC_DOWN_COUNT=$(cat "$RPC_DOWN_COUNT_FILE" 2>/dev/null || echo 0)
  echo $((RPC_DOWN_COUNT + 1)) > "$RPC_DOWN_COUNT_FILE"
else
  echo 0 > "$RPC_DOWN_COUNT_FILE"
...
# ───────────────────────────────────────────────────────────
# BFT WATCHDOG (v5): auto-recover from dead-state
# ───────────────────────────────────────────────────────────
# monad-bft can enter a dead-state where the systemd unit reports "active"
# but UDP transmission stops, peer-discovery sits idle with empty
# chosen_peers, and consensus never resumes. Observed 2026-05-05 on
# Astrosynx — `systemctl restart monad-bft` cleared it within 60s.
#
# Triggers (any of, with safety guards below):
#   - block-stuck: eth_blockNumber unchanged for >= WATCHDOG_STUCK_THRESHOLD cycles
#   - rpc-down:   RPC unreachable for >= WATCHDOG_RPC_DOWN_THRESHOLD cycles
#
# Safety guards:
#   - BFT must be systemctl-active (don't restart Failed; root cause is different)
#   - BFT uptime >= WATCHDOG_MIN_UPTIME_SEC (don't churn during legitimate warmup)
#   - WATCHDOG_COOLDOWN_SEC since last auto-restart (prevent restart loops)
WATCHDOG_FIRED=false
if systemctl is-active --quiet monad-bft; then
  BFT_ENTER_TS=$(date -d "$(systemctl show monad-bft --property=ActiveEnterTimestamp --value)" +%s 2>/dev/null || echo 0)
  NOW_TS=$(date +%s)
  BFT_UPTIME=$((NOW_TS - BFT_ENTER_TS))
  LAST_RESTART_TS=$(cat "$WATCHDOG_LAST_RESTART_FILE" 2>/dev/null || echo 0)
  TIME_SINCE_RESTART=$((NOW_TS - LAST_RESTART_TS))
  STUCK_COUNT=$(cat "$STUCK_COUNT_FILE" 2>/dev/null || echo 0)
  RPC_DOWN_COUNT=$(cat "$RPC_DOWN_COUNT_FILE" 2>/dev/null || echo 0)

  TRIGGER_REASON=""
  if [ "$STUCK_COUNT" -ge "$WATCHDOG_STUCK_THRESHOLD" ]; then
    TRIGGER_REASON="block_stuck=${RPC_BLOCK:-?}_for_${STUCK_COUNT}_cycles"
  elif [ "$RPC_DOWN_COUNT" -ge "$WATCHDOG_RPC_DOWN_THRESHOLD" ]; then
    TRIGGER_REASON="rpc_down_for_${RPC_DOWN_COUNT}_cycles"
  fi

  if [ -n "$TRIGGER_REASON" ] \
     && [ "$BFT_UPTIME" -ge "$WATCHDOG_MIN_UPTIME_SEC" ] \
     && [ "$TIME_SINCE_RESTART" -ge "$WATCHDOG_COOLDOWN_SEC" ]; then
    systemctl restart monad-bft
    echo "$NOW_TS" > "$WATCHDOG_LAST_RESTART_FILE"
    echo 0 > "$STUCK_COUNT_FILE"
    echo 0 > "$RPC_DOWN_COUNT_FILE"
    WATCHDOG_FIRED=true
    send_alert "🔧 *WATCHDOG — ${HOSTNAME}* (\`${IP}\`)
monad-bft auto-restarted
Reason: \`${TRIGGER_REASON}\`
Prior BFT uptime: ${BFT_UPTIME}s
Cooldown until next watchdog action: ${WATCHDOG_COOLDOWN_SEC}s"
  fi
fi
...
" | while IFS='|' read -r UPD_ID RCHAT TEXT; do
    # Note: UPD_ID, not UID — UID is a bash readonly built-in (user id)
    # and `read -r UID` errors out, breaking the entire bot loop.
...
UPGEOF
  ...
  # Create cron jobs — ACTIVE by default (v6: was .disabled in v3-v5)
  # Clean up any stale .disabled artifacts from prior installs
  rm -f /etc/cron.d/monad-monitor.disabled /etc/cron.d/monad-daily-report.disabled
  echo "*/3 * * * * root /usr/local/bin/monad-monitor.sh" > /etc/cron.d/monad-monitor
  echo "0 9 * * * root /usr/local/bin/monad-daily-report.sh" > /etc/cron.d/monad-daily-report
  chmod 644 /etc/cron.d/monad-monitor /etc/cron.d/monad-daily-report

  # Create bot-handler systemd unit and enable+start (v6: was manual)
...
BOTUNIT
  systemctl daemon-reload
  systemctl enable monad-bot.service >> "$LOGFILE" 2>&1
  systemctl restart monad-bot.service

  log_ok "Monitoring active: cron (3-min health + 9am daily) + monad-bot (/status)"
}
...
phase9_verify() {
  ...
  # v3 (FIX 11): disable set -e for the verification body. RPC may not be
  # listening yet (binds after first finalized block); curl gets connection
  # refused → exit 7. With set -e + pipefail, this would silently kill
  # the script before the summary box prints. We catch failures inline via
  # || echo fallbacks and accept missing values gracefully.
  set +e
...
Status: ${SYNC_LABEL}
━━━━━━━━━━━━━━━━━━━━━
Monitoring: ACTIVE (cron 3-min + daily 9am + /status bot + BFT watchdog)
Pubkeys: /opt/monad/backup/pubkeys.txt"

  # v7 (FIX 17): verify Telegram bot reachable post-bootstrap. tg_notify
  # itself swallows curl errors (best-effort by design); this gives a
  # second-chance signal in /var/log/monad-install.log so the operator
  # learns about a misconfigured token before relying on alerts.
  if [[ "$SKIP_MONITORING" == false ]]; then
    sleep 3
    local TG_OK
    TG_OK=$(curl -s --max-time 10 \
      "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/getMe" \
      | grep -c '"ok":true' || true)
    if [[ "$TG_OK" != "1" ]]; then
      log_warn "Telegram bot getMe check failed — bootstrap message may not have been delivered. Verify TELEGRAM_BOT_TOKEN/TELEGRAM_CHAT_ID."
    else
      log_ok "Telegram bot reachable (bootstrap notification dispatched)"
    fi
  fi
  ...
  echo  "║                                                                      ║"
  echo  "║ TG monitoring:   ACTIVE (auto-enabled in v6)                         ║"
  echo  "║   Health check:  every 3 min  (/etc/cron.d/monad-monitor)            ║"
  echo  "║   Daily report:  09:00 UTC    (/etc/cron.d/monad-daily-report)       ║"
  echo  "║   /status bot:   monad-bot.service (enabled+running)                 ║"
  echo  "║   BFT watchdog:  block-stuck≥9m → auto-restart bft (5m warmup)       ║"
  echo  "║                                                                      ║"
  ...
  set -e   # v3 (FIX 11): restore strict mode for any post-Phase 9 code
}
...
main() {
  ...
  phase1_system
  phase2_package
  phase2_5_otel         # v4: otelcol-contrib (one of 6 services per Monad docs)
  phase3_triedb
  phase4_keystore
  phase5_config
  phase6_systemd
  ...
  phase8_start
  phase7_monitoring     # v7 (FIX 18): moved AFTER phase8_start, BEFORE phase9_verify.
                        # v6 ran phase7 before snapshot → cron monad-monitor.sh fired
                        # during 10-30min snapshot download, spammed false "service down"
                        # alerts. Now monitoring activates only when node is up AND before
                        # the bootstrap-complete TG notification, so the message's
                        # "Monitoring: ACTIVE" claim is accurate at send time.
  phase9_verify
}
